...
class Sigmoid(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tensor:
        ...
        return result * (_tensor([1], shape=(1,), backend=grad_output.backend) - result) * grad_output
...
class Permute(Function):
    ...
    def forward(ctx: Context, a: Tensor, order: Tensor) -> Tensor:
        ...
        permuted = a._tensor.permute(*order)
        tensor = minitorch.Tensor.make(permuted._storage, permuted.shape, permuted.strides, a.backend)
        return tensor

    @staticmethod
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, float]:
        (a, order,) = ctx.saved_values
        return minitorch.Tensor.make(grad_output._tensor._storage, a._tensor.shape, a._tensor.strides, a.backend), order.zeros()
